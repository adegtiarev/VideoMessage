...
data class TextFrameData(
    ...
    val padding: Int = 0,
    val isBold: Boolean = false,   // Добавлено
    val isItalic: Boolean = false  // Добавлено
)
...
class TextFrameProducer @Inject constructor(
    ...
    fun createFrame(data: TextFrameData): Bitmap {
        ...
        // Создаем битмап исходного размера View
        val bitmap = Bitmap.createBitmap(data.viewWidth, data.viewHeight, Bitmap.Config.ARGB_8888)
        val canvas = Canvas(bitmap)

        // 1. Заливка фона
        canvas.drawColor(data.backgroundColor)

        // 2. Настройка кисти с учетом стилей
        val textPaint = TextPaint(Paint.ANTI_ALIAS_FLAG).apply {
            textSize = data.textSizePx
            color = data.textColor
            
            // Определяем стиль Typeface
            val style = when {
